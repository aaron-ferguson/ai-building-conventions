...
- **A ratio is measured against the pixel that is actually painted, not the colour you named.** Any opacity between the text and its background — a translucent fill, an overlay, a tint that varies with state — means the source colour is not what the eye receives, and a check written against the source measures a value the product never renders. Composite it first, and cover the *range* where the opacity moves: a fill whose alpha tracks a state has its worst case in the middle, not at the ends. Fixing this by inverting the text has a hard ceiling worth knowing — a mid-luminance background clears barely 4:1 against *either* a near-black or a near-white, so nothing you choose reads on it. Keep the background out of that band instead.
