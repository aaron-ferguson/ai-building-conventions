...
**Reproducibility is the principle; the mechanism is a preference.** Declarative IaC is the target, and the lower rungs are named scale-downs, not equal alternatives (`CONVENTIONS_CORE.md` — how these conventions are calibrated). Whichever rung a project sits on is recorded in its CLAUDE.md:

1. **Declarative IaC** — Terraform, Pulumi, CloudFormation, or the platform's own config-as-code. **The default.** State is described rather than performed, so drift is detectable and re-applying is safe. Reach for this first, including on small projects — most platforms now have a config-as-code path that costs an afternoon.
2. **Scripts** — the same steps as executable, idempotent code (provider CLI, a setup script). A legitimate scale-down when the platform has no real IaC story, or the surface is two or three resources. It can't drift from itself, but it can't tell you when reality has drifted from *it*.
3. **A written runbook** — the exact steps and settings, in the repo, kept current. The floor, and only defensible while the whole environment is a handful of clicks. Its failure mode is going stale silently, so it gets re-read and corrected every single time it's used.

Move up a rung when a trigger fires: a second environment, a second person, `release: released`, or the first time you can't confidently answer "what exactly is deployed?" Going *down* a rung is a deliberate, written decision, not something that happens because a runbook was quicker that day.
...
- **An emergency console change is allowed; leaving it there is not.** Fixing production by hand at 2am is sometimes correct. The change is reconciled back into the definition — and into staging — before the incident is closed, not "when there's time." That reconciliation is a named close-out item in `incident-conventions.md`; `environment-conventions.md` states the same rule as changes flowing forward, never sideways into production only.
...
- **Three copies, two kinds of storage, one of them off-site and outside the production account.** That's the 3-2-1 rule, and it's the default target here rather than an enterprise luxury — cloud object storage in a second account or region makes it genuinely cheap. The principle underneath it, which is what you scale down *to* and never below: **no single event — a bad migration, a deleted account, a compromised credential, a provider region failure — may be able to destroy both the data and every copy of it.** A snapshot living in the same account as the database it protects fails that test, and is the most common way a backup strategy turns out not to be one.
