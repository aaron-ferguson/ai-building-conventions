...
- **A real project's name, or a machine-specific path, used as a worked example.** The same failure as the state rule above, from a different angle: a fact true of one project or one install, written into a durable doc as though it were the general case. A reader can't tell it apart from the real thing, and on a shared or public repo it publishes an association nobody intended. Use an obviously fake stand-in (`widget-utils`, `/path/to/project`) instead.
