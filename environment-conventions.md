...
**Rung 2+** — rung 2 plus an automated check run against production the moment a deploy lands — is the honest option for a project with no staging, where the real alternative is verifying nothing. It is weaker than rung 3: it catches the same failures *after* users could reach them. Legitimate only if the check is automated and specific (not "the site loads"), rollback is one known-good action, and deploys happen when the blast radius is smallest.

**Two things no environment below production can verify**, and the reason rung 2+ needs a check rather than confidence:

- **Deploy-time platform config** — headers, redirects, caching, CSP, anything the host applies from `vercel.json` / `netlify.toml` / an ingress. If the dev server doesn't apply them, nothing local or in CI can. Staging on the same platform does verify these, which is among the clearest things it buys.
- **Paths reachable only in production** — the branch selected by the production hostname, a production-only credential. Unexercisable anywhere else by construction, so they also need a design that fails loudly (`coding-conventions.md`).
...
**Fill this block in from the platform, not from existing docs.** A transcribed deploy trigger nobody has re-checked silently corrupts the push rule in both directions. If you can't confirm one, record it as unconfirmed — that resolves to "gated"; a confidently wrong one doesn't.
...
- **Namespace the variables the project reads, and reject a partial set.** Generic names (`DATABASE_URL`, `API_KEY`) collide with whatever else is exported in the same shell. Prefix per project, and where several must agree require all or none — a half-configured target silently points at something nobody chose.
...
Environment count and naming (`dev`/`test`/`uat`/`pre-prod`), promotion gates, sign-off requirements, and hosting per environment are company-specific — they live in the company profile (see `companies/_template.md`), where they may only tighten what's here. Project-specific deviations go in that project's CLAUDE.md and take precedence over the general defaults, never over the principles: local exists and is isolated, non-production never holds production data or credentials, and a released project's changes are verified somewhere production-like before users get them.
