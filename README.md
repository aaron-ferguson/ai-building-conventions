...
- **Check the diff size before committing.** Lessons from one session growing a file by more than a few lines means the reasoning went in with the rule. Compress, then commit — and **compress by naming what leaves first**, a category or a section belonging in another file, before shaving words. Word-level passes move a fraction of what moving one category does, and take several passes to reveal it.
