...
- **Stage specific files** — `git add <file>`, never `git add .`, `git add -A`, or `git commit -a`. It avoids committing `.env` files, secrets and build artifacts; and where more than one agent or window works a repo at once, **the index is shared and unguarded**, so a swept commit silently carries off whatever the other session had staged. Enforced by `hooks/block-sweeping-git-stage.sh` where that hook is wired in (see `README.md` → "Adopting the hooks").
...
- **Never bare `git stash`** in a shared tree — it takes the other session's uncommitted work with it. Scope it (`git stash push -u <path>…`) or don't stash. Enforced by `hooks/block-sweeping-git-stage.sh` where that hook is wired in.
